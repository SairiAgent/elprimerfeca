...
Segun `docs/daily-reels-automation.md`, `script.txt` alimenta tanto el audio generado por el modelo de video como los subtitulos quemados en postproduccion; deberian decir lo mismo. Este asset se genero fuera del flujo `media/daily/**` documentado en este repo (commit `00fadf2`, autor `SairiAgent`, carpeta `assets/reels-2026-09-15/` + `assets/portal-*-2026-09-15/`), asi que no hay `script.txt` ni `video-prompt.txt` versionados para reconstruir que paso. La evidencia (contenido extra en el audio que no esta en ningun cartel, mas nombres propios comidos) apunta a que el audio quedo de una version anterior del guion y no se re-genero tras el recorte/edicion final.
